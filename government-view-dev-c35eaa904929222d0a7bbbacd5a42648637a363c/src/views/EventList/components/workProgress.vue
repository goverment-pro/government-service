...
<!--                    </el-upload>-->
        ...
        <el-dialog title="进展修改记录" :visible.sync="dialogWorkProgress">
            <el-table
                    :data="progressRecordData"
                    :header-cell-style="headClass"
                    border
            >
                <el-table-column property="operator" label="操作员" width="150"></el-table-column>
                <el-table-column property="operateType" label="操作类型" width="150"></el-table-column>
                <el-table-column property="operateContent" label="操作内容"></el-table-column>
                <el-table-column property="operateTime" label="操作时间" width="150"></el-table-column>
            </el-table>
        </el-dialog>
    </div>
</template>
...
<script>
import { deleteProgress, fetchSelectList, addWorkProgress, queryWorkProgress, uploadFile, queryEventProgressModifyRecord } from '@/api/eventManage.js'
export default {
  name: 'workProgress',
  props: ['origin', 'form'],
  data () {
  ...
  methods: {
    viewWorkProgress () {
      ...
      queryWorkProgress({ eventId: this.form.eventId })
        .then((data) => {
          console.log(data)
    ...
    queryProgressRecord () {
      queryEventProgressModifyRecord({ eventId: this.form.eventId })
        .then(data => {
          console.log(data)
  ...
  created () {
    this.viewWorkProgress()
    this.isDialog = true
    // this.isDialog = this.form.isDialog
    ...
    fetchSelectList({})
      .then(({ data }) => {
        ...
        this.contentType = data.data.EVENT_CONTENT_TYPE
        this.operateType = data.data.OPERATE_TYPE
      })
    this.queryProgressRecord()
  }
}
</script>
